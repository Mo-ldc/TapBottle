...
export const methods: Record<string, (...args: any[]) => any> = {
    ...
    /** Returns a selected node with a stable hierarchy path; read-only. */
    getSelectedNodeDetails(uuid: string) {
        try {
            const node = findNode(uuid);
            if (!node) return { success: false, error: `Node ${uuid} not found in the active scene or prefab stage` };
            ...
            const data = collectNodeInfo(node, true);
            data.path = names.join("/");
            data.isSceneRoot = !node.parent || node.parent === getScene();
