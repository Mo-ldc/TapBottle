...
import { addEditorContext, clearEditorContext, getEditorContext, removeEditorContext } from "./context-store";
...
function createServer(config: ServerConfig): McpServer {
    ...
    s.register(new SelectionTools());
    s.register(new ContextTools());
    return s;
}
...
export const methods: Record<string, (...args: any[]) => any> = {
    ...
    addEditorContext(item: any) {
        return addEditorContext(item);
    },

    removeEditorContext(id: string) {
        return { removed: removeEditorContext(id) };
    },
