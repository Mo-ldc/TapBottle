...
import { TEXT_SS, label, nd, roundedPanel, setFrame, sliced, tint } from './UIKit';
...
export function makeCell(parent: Node, x: number, y: number, spec: RowSpec, onRefresh?: () => void): RowUI {
    ...
    const btn = sliced(root, 'ui/card_white', BTN_W, BTN_H, BTN_X, 0, [24, 24, 24, 24], TONE[spec.tone].bg, 'btn').node;
    ...
    const curSp = setFrame(curNode.addComponent(Sprite), 'ui/coin', CUR_ICON_H, CUR_ICON_H);
    ...
    adNode.active = false;
    const adSp = setFrame(adNode.addComponent(Sprite), 'ui/ksp', AD_ICON, AD_ICON);
...
export function applyRow(r: RowUI, spec: RowSpec) {
    ...
    if (r.curSp && r.curSp.isValid) {
        ...
        if (cur) { setFrameH(r.curSp, cur === 'cap' ? 'bottle/capchip_6' : 'ui/coin', CUR_ICON_H); }
    }
