...
const NINE = {
    plate: { base: 'ui/nine/nine_base', gloss: 'ui/nine/nine_gloss', stroke: 'ui/nine/nine_stroke', inset: 26 },
    chip: { base: 'ui/nine/nine_chip', gloss: 'ui/nine/nine_chip_gloss', stroke: 'ui/nine/nine_chip_stroke', inset: 27 },
};
...
export function woodRail(parent: Node, w: number, h: number, x: number, y: number, name = 'rail', color = '#D8914A'): Node {
    const n = nd(parent, name, w, h, x, y);
    const sp = n.addComponent(Sprite);
    setFrame(sp, 'ui/deco/wood_rail', w, h, color);
    sp.type = Sprite.Type.SIMPLE;
